...
public class EFAccountService : IAccountService<ApplicationUser> {
  ...
  public async Task SignInUsingUserNameOrEmail(string EmailOrUserName,
                                               string Password) {
    ...
    if (_emailAddressAttribute.IsValid(EmailOrUserName)) {
      await SignInUsingEmail(EmailOrUserName, Password);
    } else {
      await SignInUsingUserName(EmailOrUserName, Password);
    }
    return;
  }
